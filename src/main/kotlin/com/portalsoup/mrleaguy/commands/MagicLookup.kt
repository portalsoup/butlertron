...
class MagicLookup : AbstractCommand() {
    ...
    override fun runPredicate(event: GuildMessageReceivedEvent): Boolean {
        return event.message.contentRaw.toLowerCase().startsWith("mtg")
    }

    override fun run(event: GuildMessageReceivedEvent) {
        val url = getCardUrl(event.message.contentRaw
            .toLowerCase()
            .substring(3)
            .replace(" ", "+")
        )
...
//        val rawResponse = URL(url +
//                fuzzyName
//                    .toLowerCase()
//                    .replace(" ", "+"))
//            .readText()

        val cardUrl = JSONObject(sampleResponse).getJSONObject("image_uris").getString("normal")
...
fun main(args: Array<String>) {
    val url = MagicLookup().getCardUrl("fist of krosa")
